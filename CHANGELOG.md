...
- Replaced the linear one-sided-G1 realization factor in future-grandparent scoring with a configurable binary Independent Training win cutoff (60% by default). Race viability now uses the target parent's exact costume aptitudes plus the initial Pink-Spark gains from both complete GP branches, the MDB race distance/surface, and the documented penalties from the third consecutive race onward. The planning view and exports expose base/effective chances and races rejected by the cutoff.
- Replaced the separate Lineage Optimisation and uma.moe pages with one Search workspace: a persistent Ace/target-parent/race context, explicit local and remote source cards, three independent local actions for final pairs, parent branches and future grandparents, and one shared result area without nested tabs or sliding configuration panels. Moved the stable uma.moe API URL/key into Settings, split remote Parent/Grandparent filter and JSON-import menus, and restored full local Spark/ancestry previews by resolving compact lineage references against the linked collection.
- Moved Scenario Sparks from the flat Race/Scenario utility into the Blue component. Their actual stat targets come from `master.mdb`, their expected value uses the 3/6/9% per-event rates, individual affinity and both Inspiration Events, and they can now lift Blue above 100 without consuming another Blue slot. The same model applies to future-grandparent heuristics; Race Sparks keep their separate small stat utility and probability-scored granted skills.
...
- Reworked that calendar into an optimal one-race-per-turn schedule: fixed character objectives from `master.mdb` are locked into the standard plan, objective conflicts remain visible, and a **Schedule for Trackblazer** switch exposes the objective-free alternative. One-sided G1 races are labelled local or remote, four-race streaks remain visible with a risk treatment, and the Planning tab now uses the application's restrained dark surfaces instead of reproducing the game's schedule styling.
